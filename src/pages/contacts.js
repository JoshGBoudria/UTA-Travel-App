/* 
 * Shane Purdy 1001789955
 * Bryson Neel 1001627866
 * 
 */

import React from "react";

const Contacts = () =>
{
	// UI display
	return (
		<div>
			{/*Create a header for the Emergency Contacts tab.*/}
			<h2> Emergency Contacts </h2>
			<h3>On Call International</h3>
			<p>
				On Call International is the emergency assistance provider for UT System with an
				international network of medical and security professionals available any time to
				assist you. They have a dedicated number at&nbsp;
				<a href="tel:19786519722">+1 978-651-9722</a>
				&nbsp;for UT students abroad.
			</p>
			<h3>U.S. Department of State Overseas Citizens Services</h3>
			<p>
				Contact the Overseas Citizens Services at&nbsp;
				<a href="tel:12025014444">+1 202-501-4444</a>
				&nbsp;in the case of an emergency abroad. The Overseas Citizens Office can assist
				you in the event that you are arrested or detained abroad, are robbed, or if you need
				to report the death or disappearance of a fellow U.S. citizen abroad.
			</p>
			<h3>Contact Information for UTA</h3>
			<p>
				The UTA Study Abroad Office remains available to you in an emergency 24 hours a day.
				We can be reached Monday through Friday, 8am to 5pm by calling&nbsp;
				<a href="tel:18172721120">+1 817-272-1120</a>
				. If you need immediate assistance outside of office hours, please call the UTA
				Police at&nbsp;
				<a href="tel:18172723003">+1 817-272-3003</a>
				, and they will notify the Study Abroad Staff.
			</p>
			<h3>US Embassy/Consulates in Japan</h3>
			<p>
				Tokyo (Embassy) phone:&nbsp;
				<a href="tel:03-3224-5000">03-3224-5000</a>
				&nbsp;address: 1-10-5 AkasakaMinato-ku, Tokyo 107-8420 Japan

				<br></br>
				<br></br>

				Osaka (Consulate) phone:&nbsp;
				<a href="tel:06-6315-5900">06-6315-5900</a>
				&nbsp;address: 2-11-5, Nishitenma,Kita-ku, Osaka 530-8543
			</p>
			<p>
				And in case of Earthquake/Tsunami/National Emergency, students should download the
				NHK World App in new tab:&nbsp;
				<a href="https://www3.nhk.or.jp/nhkworld/en/app/" target="_blank"
				rel="noopener noreferrer">https://www3.nhk.or.jp/nhkworld/en/app/</a>
				&nbsp;to receive "J-Alerts" on their phone.

				<br></br>
				<br></br>
				
				Also, the US Embassy offers tips for safety on this website:&nbsp;
				<a href="https://jp.usembassy.gov/services/emergency-preparedness-americans-japan/"
				target="_blank" rel="noopener noreferrer">
				https://jp.usembassy.gov/services/emergency-preparedness-americans-japan/</a>
			</p>
		</div>
	);
};

export default Contacts;
